...
@media (max-width: 319px) { $bpcol[6] }";
        ...
        if ( ! $ajax_feed) {
            ...
            if ( ! $is_feed && (@$attr['position'] == 'normal' || ! $is_timeline && ! @$attr['tabable'] ) ) {
                ...
                if ( @$attr['filter_search'] )
                    $filter_box .= '<input type="text" class="sb-search" placeholder="'.ss_lang('Search...', 'social-board').'" />';
                if ($filter_box) {
                    $output .= '
            ...
            foreach ($feeds as $feed_class => $feeditem) {
                ...
                if ($feed_class == 'facebook') {
                    ...
                    foreach ($data as $entry) {
                        ...
                        $itemdata = array(
                        'title' => (@$title) ? '<a href="' . (@$entry->link ? $entry->link : $link) . '"'.$target.'>' . (@$attr['titles'] ? $this->title_limiter($title) : $title) . '</a>' : '',
                        'thumb' => (@$source) ? $source : '',
                        'thumburl' => $url,
                        'text' => @$text,
                        'meta' => @$meta,
        ...
        $more_output .= '
        jQuery(".sboard").on("click", ".sb-fetchcomments a", function() {';
        $more_output .= '
            fcobj = $(this).parent();
        ...
        $more_output .= '
            ...
            .fail(function() {
                fcobj.html(\'<a href="javascript:void(0)">'.ss_lang( 'show_comments' ).'</a>\');
            ...
            case 'instagram':
                ...
                if ($i == 1) {
                    ...
                    if ( ! empty($feed_token) ) {
                        ...
                        $feed_url = 'https://api.instagram.com/v1/users/' . $feed_value . '/media/recent?count=' . $results;
                    }
                } elseif ($i == 2) {
                    // Deprecated - Just to be used as cache label
                    ...
                    // Deprecated - Just to be used as cache label
                    $feed_url = 'https://api.instagram.com/v1/locations/' . $feed_value . '/media/recent';
                } elseif ($i == 4) {
                ...
                if (@$feed_url) {
                    ...
                    if ($i == 2 || $i == 3 || ($i == 1 AND empty($feed_token) ) ) {
                        $get_feed = TRUE;
                        if ( ! $forceCrawl ) {
                        ...
                        if ($get_feed) {
                            ...
                            try {
                                ...
                                $this->instagram_set_proxy();

                                $feed = new stdClass();
                                if ($i == 2) {
                                    $content = $medias = $instagram->getMediasByTag($feed_value, $results, $next_max_id);
                                } elseif ($i == 3) {
                                    $medias = $instagram->getPaginateMediasByLocationId($feed_value, $next_max_id);
                                } else {
                                    if ( ! is_numeric($feed_value) ) {
                                        $igaccount = $instagram->getAccount($feed_value);
                                ...
                                foreach ($medias as $key => $media) {
                                    ...
                                    if ( $feed->data[$key]->user->id = $account->getId() ) {
                                        if ( empty( $account->getUsername() ) ) {
                                            if ( ! isset($user_info) ) {
                                                $user_info_endpoint = "https://i.instagram.com/api/v1/users/{$feed->data[$key]->user->id}/info/";
                                            ...
                                            if ( ! empty($user_info) && $user_info->status == 'ok') {
                                                $feed->data[$key]->user->username = $user_info->user->username;
                                                $feed->data[$key]->user->full_name = @$user_info->user->full_name;
                                                $feed->data[$key]->user->profile_picture = $user_info->user->profile_pic_url;
                                            }
                                        } else {
    ...
    function add_links($endpoint, $text) {
        ...
        $text = preg_replace('/@([^\s\<.*?>]+)/', '<a href="'.$endpoints[$endpoint][0].'"'.$this->target.'>@$1</a>', $text);
        // Add links to all hash tags
        $text = preg_replace('/#([^\s\<.*?>]+)/', '<a href="'.$endpoints[$endpoint][1].'"'.$this->target.'>#$1</a>', $text);
    ...
    function instagram_is_access_token($token) {
        ...
        if ( count($tokenArr) == 3 ) {
            if ( is_numeric($tokenArr[0]) && ! is_numeric($tokenArr[2]) )
                return true;
        }
        return false;
    }
    ...
    function instagram_access_token($feed_value) {
        $instagram_access_token = ! empty($GLOBALS['api']['instagram']['instagram_accounts'])
            ? reset($GLOBALS['api']['instagram']['instagram_accounts']) : @$GLOBALS['api']['instagram']['instagram_access_token'];
        $feed_token = '';
    ...
    function instagram_set_proxy() {
        if ( ! empty($GLOBALS['api']['instagram']['instagram_login_user']) ) {
            $instagram = \InstagramScraper\Instagram::withCredentials($GLOBALS['api']['instagram']['instagram_login_user'], $GLOBALS['api']['instagram']['instagram_login_pass'], SB_DIR . '/cache/');
            $instagram->login();
        }
