...
class MasterController extends Controller
{
    ...
    public function ajaxEquiposByPais($pais){
        //cargo los equipos
        $resultado = \App\Models\MongoDB\Club::where('Pais', new ObjectID($pais) )->orderBy(
            'Nombre', 'asc')->get();
    ...
    public function ajaxEventosByEmpresa($empresa){
        //cargo los eventos
        $resultado = \App\Models\MongoDB\Evento::borrado(false)->activo(true)->where(
            'Empresa_id', new ObjectID($empresa) )->orderBy('Nombre', 'asc')->get();
    ...
    public function ajaxDepartamentos($provincia){
        ...
        $resultado = \App\Models\MongoDB\Departamento::where('Borrado', false)->where(
            'Provincia', new ObjectID($provincia) )->orderBy('Departamento', 'asc')->get();
        //devulevo un json con la data
        return json_encode($resultado);
    }
    ...
    public function ajaxLocalidades($departamento){
        ...
        $resultado = \App\Models\MongoDB\Localidad::where('Borrado', false)->where(
            'Departamento', new ObjectID($departamento) )->orderBy('Localidad', 'asc')->get();
        //devulevo un json con la data
        return json_encode($resultado);
    }
    ...
    public function ajaxSucursales($empresa){
        ...
        if($rol == 'ADMINISTRADOR' OR $rol == 'SUPERVISOR'){
            ...
            $resultado = \App\Models\MongoDB\Sucursal::where('Borrado', false)->where(
                'Empresa_id', new ObjectID($empresa) )->where('Activo', true)->orderBy(
                    'Nombre', 'asc')->get();
    ...
    public function ajaxProductosExtras($sucursal){
        ...
        //cargo los productos
        $resultado = \App\Models\MongoDB\Producto::borrado(false)->activo(true)->where('Items', 'exists', false)->whereNotIn('Categoria_id', $cat )->where(
            'Sucursal_id', new ObjectId($sucursal))->orderBy('Nombre', 'asc')->get(['_id', 'Nombre']);
        //devulevo un json con la data
        return json_encode($resultado);
    }
    ...
    public function ajaxGaleria(Request $request, $tipo){
        ...
        if($search){
            $data = Galeria::where('Tipo', $tipo)->where(
                'Nombre', 'like', '%' . $search . '%')->orderBy('Nombre')->paginate(5);
        }else{
