...
class ScheduleList
{
    ...
    public function all(): array
    {
        ...
        foreach ($this->schedule->events() as $event) {
            ...
            if (empty($scheduleEvent->getDescription()) && $scheduleEvent->getCommandName() && empty($scheduleEvent->getName())) {
                $scheduleEvent->setDescription($this->getDescriptionFromCommand($scheduleEvent->getCommandName()));
                $scheduleEvent->setName($this->getNameFromCommand($scheduleEvent->getCommandName()));
            }
    ...
    /**
     * Parse the name from the Command instead of the scheduled event
     *
     * @param string $commandName
     * @return string
     */

    private function getNameFromCommand(string $commandName): string
    {
        $commands = $this->consoleKernel->all();
        if (!isset($commands[$commandName])) {
            return '';
        ...
        try {
            $className = get_class($commands[$commandName]);
            $reflection = new \ReflectionClass($className);
            return (string)$reflection->getDefaultProperties()['name'];
        } catch (\ReflectionException $exception) {
            return '';
        }
    }
